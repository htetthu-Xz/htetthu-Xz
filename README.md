# Htet Aung Kyaw

**Full-Stack Web Developer**
Building modern web applications with Laravel & Vue.js.
[Portfolio](https://devhtetaungkyaw.netlify.app/) · [View My Work](https://devhtetaungkyaw.netlify.app/#work) · [Download CV](https://devhtetaungkyaw.netlify.app/Htet-Aung-Kyaw-CV.pdf) · [Contact](mailto:dev.htetaungkyaw@gmail.com)
```js
const developer = {
  name: 'Htet Aung Kyaw',
  role: 'Full-Stack Web Developer',
  core: ['Laravel', 'Vue.js', 'PHP', 'MySQL'],
}
function build() {
  return api + interface + data
}
```
---
## `01` About
Final-year Computer Science student at the University of Computer Studies, Hinthada, expected to graduate in March 2027, with over two years of hands-on web development experience. Experienced in building web applications using Laravel, PHP, and Vue.js, with professional experience in company projects and additional experience through freelance and personal projects. Currently seeking a university internship and Full-Stack Developer opportunities where I can contribute my skills and continue developing professionally.
| Based in | Focus | Open to |
| --- | --- | --- |
| Yangon, Myanmar | Laravel · PHP · Vue.js | Internship & fullstack roles |
---
## `02` Tech stack
**Core**
![Laravel](https://img.shields.io/badge/Laravel-07090c?style=flat-square&logo=laravel&logoColor=22d3ee)
![Vue.js](https://img.shields.io/badge/Vue.js-07090c?style=flat-square&logo=vuedotjs&logoColor=22d3ee)
![PHP](https://img.shields.io/badge/PHP-07090c?style=flat-square&logo=php&logoColor=22d3ee)
![JavaScript](https://img.shields.io/badge/JavaScript-07090c?style=flat-square&logo=javascript&logoColor=22d3ee)
![MySQL](https://img.shields.io/badge/MySQL-07090c?style=flat-square&logo=mysql&logoColor=22d3ee)

**Frontend**
![HTML5](https://img.shields.io/badge/HTML5-07090c?style=flat-square&logo=html5&logoColor=22d3ee)
![CSS3](https://img.shields.io/badge/CSS3-07090c?style=flat-square&logo=css&logoColor=22d3ee)
![Tailwind CSS](https://img.shields.io/badge/Tailwind%20CSS-07090c?style=flat-square&logo=tailwindcss&logoColor=22d3ee)
![Bootstrap](https://img.shields.io/badge/Bootstrap-07090c?style=flat-square&logo=bootstrap&logoColor=22d3ee)
![jQuery](https://img.shields.io/badge/jQuery-07090c?style=flat-square&logo=jquery&logoColor=22d3ee)
![Vite](https://img.shields.io/badge/Vite-07090c?style=flat-square&logo=vite&logoColor=22d3ee)
![Axios](https://img.shields.io/badge/Axios-07090c?style=flat-square&logo=axios&logoColor=22d3ee)
**Backend / Architecture**
![REST API](https://img.shields.io/badge/REST%20API-07090c?style=flat-square&logoColor=22d3ee)
![MVC](https://img.shields.io/badge/MVC-07090c?style=flat-square&logoColor=22d3ee)
![OOP](https://img.shields.io/badge/OOP-07090c?style=flat-square&logoColor=22d3ee)
![Laravel Sanctum](https://img.shields.io/badge/Laravel%20Sanctum-07090c?style=flat-square&logo=laravel&logoColor=22d3ee)

**Tools**
![Git](https://img.shields.io/badge/Git-07090c?style=flat-square&logo=git&logoColor=22d3ee)
![Linux](https://img.shields.io/badge/Linux-07090c?style=flat-square&logo=linux&logoColor=22d3ee)
---
## `03` Featured projects
### Digital Signature & Approval
`Enrich Insights · Team contributor` · Dec 2023 – Apr 2025
Approval system for documents, leave, invoices, and contracts. Full-stack development contribution. Primary focus on backend development. Frontend UI implementation and improvements.
Laravel · jQuery · Bootstrap · Tailwind CSS · Node.js · MySQL
[Portfolio](https://devhtetaungkyaw.netlify.app/#work)
### Shwe Myanmar Rice Mills POS
`Freelance · Solo` · Jul 2026 – Present
Solo-developed system for rice mill staff. Designed and implemented core system functionality.
Laravel API · Vue 3 · Pinia · Sanctum · Tailwind CSS · MySQL
[Portfolio](https://devhtetaungkyaw.netlify.app/#work) · [GitHub](https://github.com/htetthu-Xz/shwe_myanmar_pos_admin)
### Personal projects
Learning, assignment and freelance works. These are not production systems.
| Project | |
| --- | --- |
| Laundry Nurse POS | [GitHub](https://github.com/htetthu-Xz/laundry_nurse_pos) |
| E-Commerce Site | [GitHub](https://github.com/htetthu00/e-comm-pj) |
| Online Course Selling | [GitHub](https://github.com/htetthu00/Tech_X_App) |
| Movie Channel | [GitHub](https://github.com/htetthu00/movies-store-app) |
| Material Inventory Checker | [GitHub](https://github.com/htetthu-Xz/materialhub) |
---
## `04` Experience
**Junior Developer**  
Enrich Insights Consulting Services Co., Ltd  
`Dec 2023 – Apr 2025`
Contributed to the development of a company approval management application, focusing primarily on Laravel backend development while also supporting frontend UI implementation.
HTML · CSS · JavaScript · jQuery · Bootstrap · Tailwind CSS · PHP · Laravel · Vite · MySQL · API
**Freelance Software Developer**  
Freelance  
`Apr 2025 – Present`
Independently developed full-stack solutions for freelance client work and academic projects, with hands-on experience in Vue.js alongside backend development.
Laravel API · Vue 3 · Pinia · Sanctum · Tailwind CSS · MySQL
**Assistant Network Engineer**  
Myanmar Net  
`Feb 2022 – May 2022`
Fiber installation engineer for Myanmar Net, responsible for installing and maintaining fiber optic networks.
---
## `05` Education
**Bachelor of Computer Science**  
University of Computer Studies Hinthada  
`Expected graduation: March 2027`
- Data Structures & Algorithms
- Database Management Systems (DBMS)
- Software Engineering
- Operating Systems
**Diploma in Web Development**  
KMD Institute, Yangon  
`July 2022 – June 2023`
---
## `06` Let's work together
Open to a Full-Stack Developer opportunities. Email is the fastest way to reach me.
| | | |
| --- | --- | --- |
| Email | [dev.htetaungkyaw@gmail.com](mailto:dev.htetaungkyaw@gmail.com) | Best for opportunities |
| Phone | [09957054088](tel:+959957054088) | Yangon, Myanmar |
| GitHub | [github.com/htetthu-Xz](https://github.com/htetthu-Xz) | Code and repositories |
| LinkedIn | [linkedin.com/in/htetthu-xz](https://www.linkedin.com/in/htetthu-xz/) | Professional profile |
| Website | [HtetAungKyaw-Porifolio](https://www.devhtetaungkyaw.netlify.app) | Professional Web profile |
© 2026 Htet Aung Kyaw. All rights reserved.
